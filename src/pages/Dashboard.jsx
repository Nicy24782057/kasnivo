...
function Dashboard() {
  ...
  const [aiSelectedAccountId, setAiSelectedAccountId] = useState('')
  const [aiFromAccountId, setAiFromAccountId] = useState('')
  const [aiToAccountId, setAiToAccountId] = useState('')
  ...
  function getAccountName(id) {
    ...
    return account ? account.name : 'Akun'
  }
  ...
  const totalTransfers = useMemo(() => {
    ...
    currentMonthTransactions.forEach((transaction) => {
      if (transaction.transfer_group_id) {
        groups.add(transaction.transfer_group_id)
      }
    })
  ...
  const expenseByCategory = useMemo(() => {
    ...
    realMonthTransactions
      ...
      .forEach((transaction) => {
        ...
        map[category] += Number(transaction.amount)
      })
  ...
  async function handleAnalyzeAI() {
    ...
    if (!text) {
      setAiError('Tulis transaksi terlebih dahulu.')
      return
    }
    ...
    try {
      ...
      if (data.kind === 'transaction') {
        setAiSelectedAccountId(
          findAccountIdByName(data.account_name)
        )
      }
  ...
  async function handleSaveAIResult() {
    ...
    if (amount <= 0) {
      setAiError('Nominal transaksi belum valid.')
      return
    }
    ...
    try {
      ...
      if (aiResult.kind === 'transaction') {
        if (!aiSelectedAccountId) {
          setAiError(
        ...
        const { error } = await supabase
          ...
          .insert({
            ...
            type: aiResult.transaction_type,
            amount,
      ...
      } else if (aiResult.kind === 'transfer') {
        if (
          !aiFromAccountId ||
          !aiToAccountId
        ) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-7">
        ...
        <div className="bg-white border border-slate-200 rounded-[26px] p-5">
          <div className="flex items-center justify-between">
            ...
            <div className="w-10 h-10 bg-slate-100 text-slate-600 rounded-2xl flex items-center justify-center font-bold">
              ≡
            </div>
          </div>
      ...
      <div className="mb-7 bg-gradient-to-br from-emerald-50 via-white to-white border border-emerald-100 rounded-[30px] p-5 sm:p-7">
        <div className="flex items-start gap-4">
          <div className="hidden sm:flex w-12 h-12 shrink-0 rounded-2xl bg-emerald-500 text-white items-center justify-center font-bold text-lg">
            ✦
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {aiSuccess && (
              <div className="mt-5 bg-emerald-50 border border-emerald-100 rounded-2xl p-4 text-sm text-emerald-700">
                {aiSuccess}
              </div>
            )}
        ...
        {accounts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {accounts.map((account) => (
              <div
                key={account.id}
                className="bg-white border border-slate-200 rounded-[26px] p-5 hover:shadow-lg hover:shadow-slate-200/50 transition"
              >
